...
Disable backup files to avoid =~= files cluttering the filesystem.
...
Emacs 29+ includes =json-ts-mode= built-in. Enable it for =.json= files.
...
From https://github.com/rust-lang/rust-mode
...
[[https://github.com/hniksic/emacs-htmlize][htmlize]] is required for syntax highlighting in source blocks when exporting org files to HTML.
...
# Deprecated 2026-04-05 — replaced by Denote. See the Denote section below.
...
Permanent notes using the Denote package. The =denote-directory= is a list of
PARA-structured subdirectories under =~/Documents/Notes2/=. Singleton files
(=inbox.org=, =journal.org=, =stack.org=) live in the root and are not indexed
by Denote, but may contain =[[denote:ID]]= links.
...
Capture templates for the three singleton files. The journal template uses
=my/journal-capture-position= (defined in =reminders.el=) to insert under
today's =* YYYY-MM-DD= heading, consistent with the reminders pipeline.
...
Local Emacs modules live in =~/dotfiles/elisp/=. Each module is a domain-grouped
file ending with =(provide 'symbol)=. See =CLAUDE.md= for conventions.
